...
export const WorkloadChart = ({ data }: WorkloadChartProps) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 md:p-5 shadow-sm border border-gray-100 w-full flex-1 flex flex-col">
      <Title
        tag="h2"
        variant="light"
      ...
      {data.length === 0 ? (
        ...
        <div className="flex-1 flex flex-col w-full min-h-0">
          {/* ШАПКА ТАБЛИЦІ */}
          <div className="hidden sm:grid grid-cols-[70px_1fr_75px] items-center gap-2 w-full text-[10px] md:text-[11px] font-medium text-gray-400 mb-3 shrink-0 pb-2 border-b border-gray-50">
            <div></div>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden flex flex-col gap-5 mt-1 pb-4 custom-scrollbar w-full">
            {data.map((team, index) => {
              ...
              {
                /* 🛠️ ФІКС ДЛЯ ПРАВИХ ТУЛТІПІВ (ЧИСЛА): Перші два елементи кидаємо вниз */
              }
              ...
              return (
                <div key={`${team.team_name}_${index}`} className="w-full">
                  {/* MOBILE UI */}
                  <div className="flex justify-between items-center sm:hidden w-full mb-1 pb-2 border-b border-gray-50/50">
                    <span className="text-[13px] font-semibold text-gray-900 truncate max-w-[60%]">
                      {team.team_name}
                    </span>
                    <div className="text-[11px] whitespace-nowrap text-right flex items-baseline justify-end gap-1">
                  ...
                  {/* DESKTOP UI */}
                  <div className="hidden sm:grid grid-cols-[70px_1fr_75px] items-center gap-2 w-full relative py-0.5">
                    {/* Стовпець 1: Назва */}
                    <div className="text-[13px] md:text-sm font-semibold text-grayscale-800 truncate">
                      {team.team_name}
                    </div>
                    ...
                    {/* Стовпець 3: Числа з виправленими тултіпами */}
                    <div className="flex items-baseline text-[11px] md:text-xs whitespace-nowrap relative w-full justify-end pr-0.5">
                      {/* Відсотки */}
                      <span className="relative group w-[32px] text-right shrink-0 pr-0.5">
                        <span
                        ...
                        >
                          ...
                          <div
                            className={`absolute right-3 border-4 border-transparent ${isTopTwoItems ? "bottom-full border-b-grayscale-900" : "top-full border-t-grayscale-900"}`}
                          ></div>
                        </div>
                      </span>

                      {/* Скорочені дужки */}
                      <span className="relative group text-gray-500 cursor-default w-[42px] text-left pl-0.5 truncate">
                        {details}
                        <div
                        ...
                        >
                          ...
                          в зоні ризику
                          <div
                            className={`absolute right-4 border-4 border-transparent ${isTopTwoItems ? "bottom-full border-b-grayscale-900" : "top-full border-t-grayscale-900"}`}
                          ></div>
                        </div>
                      </span>
                    </div>
